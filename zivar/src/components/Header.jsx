...
export default function Header() {
  ...
  return (
    <>
      ...
      >
        <div className={`w-full px-4 md:px-8 lg:px-12 flex items-center justify-between relative transition-all duration-500 ${
          isScrolled ? 'min-h-[64px] py-2' : 'min-h-[75px] md:min-h-[100px] py-3'
        }`}>
          ...
          <div className="flex items-center shrink-0 z-10">
            ...
            >
              <div className="flex flex-col gap-[5px] items-center justify-center">
                <span className="block h-[1.5px] w-5 bg-[#1A1815] transition-all duration-300" />
                <span className="block h-[1.5px] w-5 bg-[#1A1815] transition-all duration-300" />
                <span className="block h-[1.5px] w-5 bg-[#1A1815] transition-all duration-300" />
              </div>
            </motion.button>
          ...
          <nav className="hidden md:flex items-center gap-4 lg:gap-8 absolute left-1/2 -translate-x-1/2 z-10 w-max max-w-[60%] flex-wrap justify-center">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollTo(item.id)}
                className="relative text-[12px] lg:text-[14px] uppercase tracking-[0.18em] lg:tracking-[0.24em] transition-colors duration-300 font-bold py-1.5 whitespace-nowrap"
              >
          ...
          {/* 3. СПРАВА: Языки + Лого */}
          <div className={`flex items-center gap-3 sm:gap-4 md:gap-8 lg:gap-12 shrink-0 z-10 transition-all duration-500 ${
            isScrolled ? 'pt-0' : 'pt-2 md:pt-0'
          }`}>
            
            {/* Языки */}
            <div className="flex items-center text-[10px] lg:text-[11px] font-bold tracking-widest sm:tracking-medium">
              {languages.map((lang, i) => (
                <span key={lang.code} className="flex items-center">
                  ...
                  {i < languages.length - 1 && (
                    <span className="text-[#C8BBA8] opacity-60 text-[10px] mx-0.5 sm:mx-1">/</span>
                  )}
                </span>
              ))}
            </div>
            ...
            >
              <img
                ...
                className={`w-auto object-contain transition-all duration-500 ease-in-out ${
                  isScrolled
                    ? 'md:opacity-0 md:h-0 md:overflow-hidden h-[60px] opacity-100'
                    : 'opacity-100 h-[75px] sm:h-[85px] md:h-[95px] lg:h-[110px]'
                }`}
              />
            </button>
          </div>
